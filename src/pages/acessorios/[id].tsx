...
export const getStaticPaths: GetStaticPaths = async () => {
  const response = await fetch(`https://teste-brazmotors.herokuapp.com/acessorios`)
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const response = await fetch(`https://teste-brazmotors.herokuapp.com/acessorios/${id}`)
  const data = await response.json();
...
export default function AcessorioPage({ acessorio }) {
  ...
  async function EnviarMensagem() {
    ...
    await fetch('https://teste-brazmotors.herokuapp.com/mensagem/AcessoriosAutomotivo', {
      method: 'POST',
  ...
  return (
    <>
    <Head>
      <title>{acessorio[0].titulo}</title>
    ...
    <Container>
      ...
      <Content>
        <UpSide>
          <Left>
            <img src={acessorio[0].imagem} />
            <LeftDown>
            <h1 style={{ color: "black", fontSize: "1.6rem" }}>{acessorio[0].titulo}</h1>
            <p>{acessorio[0].descricao}</p>
          </LeftDown>
          </Left>
          <Right>
            <h1 style={{
            ...
            <div style={{
              ...
              <span>Valor: {acessorio[0].preco.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
            </div>
