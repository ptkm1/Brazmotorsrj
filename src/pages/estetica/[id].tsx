...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const response = await fetch(`https://teste-brazmotors.herokuapp.com/estetica-automotiva/${id}`)
  const data = await response.json();
...
export default function Estetica({ estetica }) {
  ...
  async function EnviarMensagem() {
    ...
    await fetch('https://teste-brazmotors.herokuapp.com/mensagem/EsteticaAutomotiva', {
      method: 'POST',
  ...
  return (
    <>
      <Head>
        <title>{estetica[0].titulo}</title>
        <link rel="stylesheet" type="text/css" charSet="UTF-8" href="https:/import React, { useEffect } from 'react'
import { useRouter } from 'next/router'
import Head from 'next/head'
import Link from 'next/link'
import Image from 'next/image'
    ...
    <>
      ...
      <Container>
        ...
        <Content>
          <UpSide>
            <Left>
              <img src={estetica[0].imagem} />
              <LeftDown>
              <h1 style={{ color: "#e63946", fontSize: "1.6rem" }}>{estetica[0].titulo}</h1>
              <p>{estetica[0].descricao}</p>
            </LeftDown>
            </Left>
            <Right>
              <h1 style={{
